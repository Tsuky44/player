...
void main() {
  group('fixed chrome round-trips through the config JSON', () {
    test('a fixed preset keeps its chrome id across encode/decode', () {
      final config = PlayerLayoutConfig.fixed(FixedChromeId.onyx);
      final decoded = PlayerLayoutConfig.decode(config.encode());
      expect(decoded.fixedChrome, FixedChromeId.onyx);
    ...
    test('ids saved before the Chrome Onyx rename still decode', () {
      final json = PlayerLayoutConfig.standard().toJson()
        ..['fixed_chrome'] = 'emby';
      expect(PlayerLayoutConfig.fromJson(json).fixedChrome, FixedChromeId.onyx);
      expect(TimelineVisualStyle.fromId('emby'), TimelineVisualStyle.onyx);
      expect(PlayerControlTypeX.fromId('timelineEmby'),
          PlayerControlType.timelineOnyx);
      expect(PlayerControlTypeX.fromId('upNextEmby'),
          PlayerControlType.upNextOnyx);
      expect(PlayerControlTypeX.fromId('chaptersEmby'),
    ...
    test('a fixed preset carries a usable fallback layout for old clients', () {
      ...
      expect(PlayerLayoutConfig.fixed(FixedChromeId.onyx).controls, isNotEmpty);
    });

    test('copyWith keeps the chrome unless explicitly cleared', () {
      final fixed = PlayerLayoutConfig.fixed(FixedChromeId.onyx);
