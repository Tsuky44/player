...
void VideoOutput::Resize(int64_t required_width, int64_t required_height) {
  ...
  if (surface_manager_ != nullptr) {
    ...
    //
    // Onyx : sous le verrou. Le thread de rendu de Flutter lit la surface
    // (|Read|, dans le rappel de texture) pendant que ce thread-ci la
    // détruit et la recrée.
    {
      std::lock_guard<std::mutex> lock(textures_mutex_);
    ...
    auto texture_variant =
        std::make_unique<flutter::TextureVariant>(flutter::GpuSurfaceTexture(
            kFlutterDesktopGpuSurfaceTypeDxgiSharedHandle, [&](auto, auto) {
              // Onyx : |find| plutôt que |at|. L'ancienne texture, encore à
              // l'écran, est lue ici entre |RegisterTexture| — qui a déjà
              // posé le nouvel identifiant dans |texture_id_| — et son
              // |emplace|. |at| levait alors std::out_of_range sur le thread
              // de rendu, et l'exception tuait l'app dans flutter_windows.dll
              // (0xc0000409) : au lancement d'un média, par intermittence,
              // quand la texture est retaillée à l'écran.
              std::lock_guard<std::mutex> lock(textures_mutex_);
              auto texture = textures_.find(texture_id_);
  ...
  if (pixel_buffer_ != nullptr) {
    ...
    auto texture_variant = std::make_unique<flutter::TextureVariant>(
        flutter::PixelBufferTexture([&](auto, auto) {
          // Onyx : |find| plutôt que |at|, pour la même raison que la
          // texture matérielle plus haut.
          std::lock_guard<std::mutex> lock(textures_mutex_);
          auto texture = pixel_buffer_textures_.find(texture_id_);
          if (texture == pixel_buffer_textures_.end()) {
